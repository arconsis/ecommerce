...
class ShipmentProvidersRepository(
	...
	suspend fun listShipmentProviders(deliveryAddress: DeliveryAddressDto): List<ShipmentProvider> {
		...
		return when (response.status) {
			in 200..299 -> response.body<ListShipmentProviderResponseDto>()!!.providers.map {
	...
	fun getShipmentProvider(providerId: String): Uni<ShipmentProvider> {
		return shipmentProvidesRemoteStore.getShipmentProvider(providerId).map {
			when (it.status) {
				in 200..299 -> it.body<ShipmentProviderRateResponseDto>()!!.toShipmentProvider()
				else -> throw BadRequestException("Shipment providers not found")
			}
		}
	}
